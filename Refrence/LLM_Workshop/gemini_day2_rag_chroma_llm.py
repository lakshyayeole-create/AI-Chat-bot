...
4. Add your API key in the GEMINI_API_KEY variable below.
5. Never commit your real API key to GitHub.

This version uses:
    - ChromaDB        -> vector database / retrieval
    - Gemini API      -> query embeddings + LLM generation
...
#
# text-embedding-004 is a Google Gemini embedding model.
# ------------------------------------------------
...
#
# gemini-3.6-flash is Google's fast and capable Gemini model.
# ------------------------------------------------
...
# The Gemini client uses our API key to communicate with the Google GenAI API.
#
# In a real application, you would normally load the key from
# an environment variable instead of putting it directly in code.
#
# For this educational workshop, we leave the API key configuration
# above so students can clearly see where authentication is required.

if not GEMINI_API_KEY or GEMINI_API_KEY in ["YOUR_GEMINI_API_KEY_HERE", "PASTE_YOUR_KEY_HERE"]:
    raise ValueError(
        "\nPlease add your Gemini API key to your .env file (GEMINI_API_KEY=your_key) "
...
def create_query_embedding(question):
    ...
    #
    # response.embeddings[0].values looks conceptually like:
    #
    # [
    #     0.0123,
    #     -0.1821,
